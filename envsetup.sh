...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^buffer_") ; then
       CM_BUILD=$(echo -n $1 | sed -e 's/^buffer_//g')
...
function breakfast()
{
    ...
    for f in `/bin/ls vendor/buffer/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            lunch buffer_$target-userdebug
        fi
    fi
    return $?
}
...
function eat()
{
    if [ "$OUT" ] ; then
        ...
        ZIPFILE=bufferalpha-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.buffer.device=$CM_BUILD");
    then
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/buffer/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
